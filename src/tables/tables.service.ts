...
export class TablesService {
  ...
  joinTable(id: string, user: User): TableJoinResponseDto {
    ...
    if (table.players.some((player) => player.id === user.id)) {
      return { success: false, error: 'Player already joined this table' };
    }
  ...
  startGame(id: string, user: User): TableActionResponseDto {
    ...
    if (!table.players.some((player) => player.id === user.id)) {
      return { success: false, error: 'Player must join the table before starting the game' };
    }
  ...
  private addAIPlayers(table: TableDto, count: number): void {
    const maxPlayerId = Math.max(...table.players.map((player) => player.id), 1000);
