...
export class GameLogicService {
  ...
  private addLogEntry(
    ...
    table.gameLog.push({ timestamp: new Date(), type, message, data });
  }
  ...
  evaluateGameState(table: any): any {
    ...
    if (currentPlayer.isHuman && !currentPlayer.hasFolded) {
      ...
      return { table, currentPlayer, possibleActions };
    }
  }
  ...
  private evaluateEndRound(table: any): any {
    ...
    if (activePlayers.length === 1) {
      ...
      this.addLogEntry(
        table,
        'round',
        `Round ${table.round} ended. ${winner.name} wins by default`,
        { winner: winner.name, amount: table.pot },
      );
    } else {
      ...
      this.addLogEntry(table, 'round', `Round ${table.round} ended. ${winner.name} wins`, {
        winner: winner.name,
        amount: table.pot,
        hand: winner.hand,
      });
    ...
    if (humanPlayers.length === 0 || aiPlayers.length === 0 || table.players.length < 2) {
      ...
      return { table, gameOver: true, message: 'Game Over' };
    }
  ...
  private getAIAction(player: any, table: any): { type: string; amount: number } {
    ...
    if (randomActionType === 'raise') {
      ...
      amount =
        maxRaise >= minRaise
          ? Math.floor(Math.random() * (maxRaise - minRaise + 1)) + minRaise
          : minRaise;
    ...
    this.addLogEntry(
      table,
      'action',
      `AI ${player.name} chose ${randomActionType}${amount ? ' ' + amount : ''}`,
      { playerId: player.id, action: randomActionType, amount, currentPot: table.pot },
    );
  ...
  private rotateDealer(table: any): void {
    if (!table.players || table.players.length === 0) {
      this.logger.error('rotateDealer called with no players');
      return;
    }
  ...
  turnTwo(table: any): any {
    ...
    this.addLogEntry(table, 'cards', 'Flop dealt', { flop: table.river, currentPot: table.pot });
  ...
  turnThree(table: any): any {
    ...
    this.addLogEntry(table, 'cards', 'Turn dealt', {
      turnCard,
      river: table.river,
  ...
  turnFour(table: any): any {
    ...
    this.addLogEntry(table, 'cards', 'River dealt', {
      riverCard,
      river: table.river,
      currentPot: table.pot,
    });
  ...
  rotateRole(table: any): void {
    if (!table.players || table.players.length === 0) {
      this.logger.error('rotateRole called with no players');
      return;
    }
  ...
  moveToNextPlayer(table: any): void {
    ...
    if (!currentPlayer) {
      this.logger.error('moveToNextPlayer: no current player found');
      return;
    }
  ...
  executeAction(player: any, action: string, table: any, amount?: number): any {
    ...
    this.addLogEntry(
      table,
      'action',
      `Player ${player.name} (${player.id}) ${action}${amount ? ' ' + amount : ''}`,
      {
        playerId: player.id,
