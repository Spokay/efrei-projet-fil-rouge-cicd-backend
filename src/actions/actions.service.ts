...
export class ActionsService {
  private actions: ActionDto[] = [
    { name: 'fold', description: 'Fold hand' },
    { name: 'check', description: 'Check' },
    { name: 'call', description: 'Call current bet' },
    { name: 'raise', description: 'Raise bet' },
  ];
  ...
  getAvailableActions(playerId: number, tableId: number): ActionDto[] {
    ...
    return this.actions
      .filter((action) => possibleActions.includes(action.name))
      .map((action) => ({ ...action, isAvailable: true }));
  }
